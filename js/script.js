...
`;
    ...
    // Parse the XSL string into an XML document
    var xslDoc = loadXMLString(xsl);

    // Check for XSLTProcessor support
    if (window.XSLTProcessor) {
        ...
        // Set the result to the content div
        document.getElementById("facturaContent").innerHTML = resultHTML;
    } else {
...
document.getElementById('xmlFile').addEventListener('change', function() {
    const xmlFile = this.files[0];
    if (xmlFile) {
        loadFile(xmlFile, function(xmlContent) {
            var xml = loadXMLString(xmlContent);
            transformXML(xml);
        });
    }
});
